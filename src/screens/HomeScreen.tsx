...
function HomeScreen({navigation}: Props) {
  ...
  const {error: catalogueError, loading: catalogueLoading, retry: retryCatalogue} = useCatalogue();
  const rootNavigation =
    navigation.getParent<NativeStackNavigationProp<RootStackParamList>>();

  const openCategory = (category?: string) => {
    const supportedCategory = category === 'Deals' || category === 'Appliances'
      ? undefined
      : category;
  ...
  return (
    <View style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.page}>
        ...
        <CategorySection onSelectCategory={openCategory} onViewAll={() => openCategory()} />
        {catalogueLoading && (
        ...
        {catalogueError && (
          <View style={styles.catalogueState}>
            ...
            <Pressable onPress={retryCatalogue} style={styles.retryButton}><Text style={styles.retryText}>Try again</Text></Pressable>
          </View>
        )}
        <DealsSection
        ...
        <Footer
          onLinkPress={link => {
            ...
            else if (link === 'Returns Centre') rootNavigation?.navigate('Returns');
            else if (link === 'Help & Support') rootNavigation?.navigate('HelpSupport');
            else openCategory();
          }}
          onPrivacy={() => rootNavigation?.navigate('Legal', {document: 'privacy'})}
          onTerms={() => rootNavigation?.navigate('Legal', {document: 'terms'})}
        />
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  page: {flexGrow: 1},
  catalogueState: {alignItems: 'center', backgroundColor: '#FFFFFF', padding: 18},
  stateText: {color: '#69757D', fontSize: 11, marginTop: 8},
  errorText: {color: '#A52A20', fontSize: 11, textAlign: 'center'},
  retryButton: {backgroundColor: '#101820', borderRadius: 7, marginTop: 10, paddingHorizontal: 15, paddingVertical: 9},
