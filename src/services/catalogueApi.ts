...
export const catalogueApi = {
  categories: () =>
    apiGet<ApiCategory[]>('/categories', { authenticated: false }),
  products: async (params: ProductListParams = {}): Promise<ProductList> => {
    const result = await apiGet<{
      items: ApiProduct[];
      pagination: Pagination;
    }>(`/products${queryString(params)}`, { authenticated: false });
    return { ...result, items: result.items.map(mapProduct) };
  },
  product: async (identifier: string) =>
    mapProduct(
      await apiGet<ApiProduct>(`/products/${encodeURIComponent(identifier)}`, {
        authenticated: false,
      }),
    ),
  related: async (identifier: string, limit = 6) =>
    (
      await apiGet<ApiProduct[]>(
        `/products/${encodeURIComponent(identifier)}/related?limit=${limit}`,
        { authenticated: false },
      )
    ).map(mapProduct),
  reviews: (identifier: string, page = 1, limit = 10) =>
    apiGet<{
      items: ProductReview[];
  ...
  saveReview: (
    identifier: string,
    input: { rating: number; title?: string; text?: string },
  ) =>
    apiPost<ProductReview>(
