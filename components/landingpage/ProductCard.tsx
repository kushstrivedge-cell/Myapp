...
function ProductCard({ product, compact = false, onPress }: ProductCardProps) {
  ...
  return (
    <Pressable
      ...
      onPress={
        onPress ??
        (() => Alert.alert(product.name, 'Product details will open here.'))
      }
      style={({ pressed }) => [
        styles.card,
    ...
    >
      <View style={[styles.art, { backgroundColor: product.color }]}>
        ...
        <WishlistButton
          label={`${wished ? 'Remove' : 'Add'} ${product.name} ${
            wished ? 'from' : 'to'
          } wishlist`}
          onPress={async () => {
      ...
      <View style={styles.details}>
        ...
        <Pressable
          ...
          onPress={async event => {
            ...
            if (product.inStock) {
              const failure = await addItem(product);
              Alert.alert(
                failure ? 'Could not add item' : 'Added to cart',
                failure ?? `${product.name} is now in your cart.`,
              );
            } else
              Alert.alert(
                'Out of stock',
